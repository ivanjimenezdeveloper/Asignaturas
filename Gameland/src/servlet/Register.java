...
public class Register extends HttpServlet {
	...
	/**
	 * Redirige al formulario correspondiente dependiedo de si ha habido errores o
	 * no
	 */
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (user.length() == 0 || pass.length() == 0 || nombre.length() == 0 || existe == true) {
			...
			/*
			 * Si ha podido registrarlo Redirige al login y si no recarga la pagina con un
			 * error
			 */
			if (correcto == false) {
				response.sendRedirect("Register?error=1");
